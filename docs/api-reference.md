...
Returns export bundles, storage usage, job metrics, queue health, Sentry status, and remaining adapter capabilities for a project. Job metrics include totals by type/status, average completed duration, total retry attempts, retried job count, retry attempts by job type, queue counts, configured queue rate limits when present, and recent failed BullMQ job summaries when Redis queue mode is active.
