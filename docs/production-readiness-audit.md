...
|-------------|----------|--------|
| Prisma-backed repository layer | Auth sessions, workspace/project ownership, project CRUD/dashboard reads, provider-key storage, script upload/version metadata, generated script-analysis scenes/shots/assets/requirements, generation job creation/completion, and job-event writes now use Prisma when `REPOSITORY_MODE=prisma` or `NODE_ENV=production`. `repository.prisma.test.ts` forces Prisma mode with a mocked Prisma client, and `prisma validate` passes with a local Postgres URL. | Partially complete |
| Remove in-memory app state from runtime | `getStore()` is still used by graph reads, Asset Bible detail/version/reference workflows, storyboard, video, collaboration, export/import, and local workflow mirroring. Foundational repository entry points, script upload/version metadata, generated scene/shot/asset/requirement records, and job/event records are Prisma-aware. | Not complete |
...
- `npm test`: passing, 14 files and 38 tests.
