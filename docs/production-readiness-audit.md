...
|-------------|----------|--------|
| Prisma-backed repository layer | Auth sessions, workspace/project ownership, project CRUD/dashboard reads, provider-key storage, script upload/version metadata, generated script-analysis scenes/shots/assets/requirements, Asset Bible status/manual requirement mutations, typed Asset Bible detail writes and graph readback, Asset Bible version/reference writes and graph readback, Prisma graph reads for project/API surfaces, generation job creation/completion, and job-event writes now use Prisma when `REPOSITORY_MODE=prisma` or `NODE_ENV=production`. `repository.prisma.test.ts` forces Prisma mode with a mocked Prisma client, and `prisma validate` passes with a local Postgres URL. | Partially complete |
| Remove in-memory app state from runtime | `getStore()` is still used by Asset Bible merge/split/project-style workflows, storyboard, video, collaboration, export/import, and local workflow mirroring. Project/API graph reads, foundational repository entry points, script upload/version metadata, generated scene/shot/asset/requirement records, typed Asset Bible detail records, Asset Bible status/manual requirement mutations, Asset Bible version/reference records, and job/event records are Prisma-aware. | Not complete |
...
- `npm test`: passing, 14 files and 42 tests.
