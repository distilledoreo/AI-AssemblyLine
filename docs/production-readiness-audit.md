...
| BullMQ + Redis queues | `submitGenerationJob` submits to BullMQ when Redis mode is enabled; script upload/re-analysis returns pending state in Redis mode and `npm run worker` starts an analysis worker that processes `script_analysis` jobs. Asset reference generation and storyboard frame generation return queued state in Redis mode, and the image worker processes `asset_reference` and `storyboard_frame` jobs. Health checks report Redis-backed queue counts; tests avoid Redis sockets in test mode. | Partially complete |
...
- `npm test`: passing, 20 files and 60 tests.
