...
Release readiness can be checked with `npm run release:readiness`. The command is intentionally operator-run rather than a normal CI gate: it checks local provider credential shape and uses `gh secret list` to confirm the repository has the secret names required by the manual live-provider workflow. The manual workflow also runs the same readiness gate in workflow-env mode before `npm run smoke:providers`, so missing injected secrets fail before any live provider calls are attempted. Unit coverage verifies the local credential checks, GitHub secret-name parsing, workflow-env secret-name derivation, missing-secret reporting, and repository slug resolution.
