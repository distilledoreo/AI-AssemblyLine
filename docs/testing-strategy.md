...
The workflow also includes a production infrastructure preflight job with real GitHub Actions Postgres 16 and Redis 7 service containers. That job installs dependencies, generates the Prisma client, applies checked-in Prisma migrations with `npm run prisma:migrate:deploy`, runs `NODE_ENV=production npm run preflight:production` against the live service ports and writable CI storage, runs `npm run smoke:prisma-repository`, and then runs `npm run smoke:redis-queue`. The Prisma repository smoke signs in, creates a workspace and project, saves and decrypts an encrypted provider key, creates/completes a generation job, reads events back through the repository layer, persists and reads a script-analysis graph, storyboard frame/version, video clip/version, collaboration invitation/assignment/activity event, and export bundle listing, and cleans up the smoke records against the migrated Postgres database. Prisma-mode job and event paths are also unit-tested to ensure they write through Prisma without seeding the process-local job/event store. The Redis smoke submits a script-analysis job through BullMQ, reads the Redis-backed queue health snapshot, publishes a project event, and confirms that the same Redis pub/sub path used by the SSE endpoint receives it. It uses synthetic non-secret provider-key strings only to exercise the production preflight key-shape gate and repository encryption path; live OpenAI, Stability, Runway, and Google AI API calls remain restricted to the explicit smoke commands outside automated CI.

Live provider API verification is available as the manual GitHub Actions workflow `.github/workflows/live-provider-smoke.yml`. It is intentionally `workflow_dispatch` only, sources provider credentials from repository secrets, and runs `npm run smoke:providers`. The normal pull-request and `main` workflows remain keyless and never call provider APIs. The video-provider smoke checks submit a short async Runway/Google AI job and immediately verify the provider status endpoint, but do not wait for final video output.

Release readiness can be checked with `npm run release:readiness`. The command is intentionally operator-run rather than a normal CI gate: it checks local provider credential shape, uses `gh secret list` to confirm the repository has the secret names required by the manual live-provider workflow, and requires successful GitHub Actions **CI** and **Live Provider Smoke** runs for the current commit. The manual workflow also runs the same readiness gate in workflow-env mode before `npm run smoke:providers`, so missing injected secrets or a non-green current-commit CI run fail before any live provider calls are attempted. Unit coverage verifies the local credential checks, GitHub secret-name parsing, workflow-env secret-name derivation, missing-secret reporting, repository slug resolution, current-commit resolution, and workflow-run evaluation.
