...
Current implementation status: `script_analysis`, `asset_reference`, `storyboard_frame`, `video_clip`, `export`, and `import` have executable BullMQ worker processors. Runway video jobs can be submitted to the live provider and finalized by the Runway result processor after polling a completed task. In Redis queue mode, the worker registers a repeatable `provider_poll` job on the video queue every 15 seconds; that job queries `provider_submitted` and `polling` Runway video jobs from the repository and invokes the result processor. Media utility jobs are still represented in the topology and retry policy, but their worker processors are not complete yet.
