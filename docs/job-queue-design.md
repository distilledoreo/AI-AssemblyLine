...
Current implementation status: `script_analysis`, `asset_reference`, `storyboard_frame`, `video_clip`, `thumbnail`, `media_convert`, `export`, and `import` have executable BullMQ worker processors. Runway video jobs can be submitted to the live provider and finalized by the Runway result processor after polling a completed task. In Redis queue mode, the worker registers a repeatable `provider_poll` job on the video queue every 15 seconds; that job queries `provider_submitted` and `polling` Runway video jobs from the repository and invokes the result processor. Worker startup awaits repeatable poll registration so scheduling failures fail startup instead of silently disabling Runway finalization. Media utility jobs require `ffmpeg` on `PATH`; `media_convert` writes the requested output file and then probes it for metadata, while `thumbnail` extracts a still frame to the requested output path.
