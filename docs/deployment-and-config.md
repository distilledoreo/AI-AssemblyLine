...
Unexpected API errors are passed through the structured `captureError` path before the route returns a `500` response. When `SENTRY_DSN` is configured, the captured log records include `sentryEnabled: true` so deployment monitoring can verify the error-tracking path is active. Expected application errors such as Zod request-validation failures, authorization, and not-found responses are returned without error capture; validation failures return `400` with `validation_error` and issue paths.
