...
Run `npm run release:readiness` before release handoff. It loads the same local env files as the provider smoke commands, verifies that local live-provider credentials are not missing/mock/placeholder values, resolves the GitHub repository from `GITHUB_REPOSITORY` or the `origin` remote, and checks `gh secret list` for the provider secret names needed by **Live Provider Smoke**. Inside the manual GitHub Actions workflow, the same command runs in workflow-env mode and verifies the provider secrets after GitHub injects them into environment variables. It reports only secret names and readiness status, never secret values.
