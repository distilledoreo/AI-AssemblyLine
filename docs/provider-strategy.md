...
Runtime generation paths resolve OpenAI credentials from the project workspace's encrypted provider key first. If no workspace key is configured, they fall back to `OPENAI_API_KEY`. Provider keys are trimmed before use, so whitespace around a saved key or environment key is not sent to the provider and any case variant of `mock` is still treated as mock mode. If neither real key is present, local development and tests use the mock OpenAI adapter response; production generation fails with a provider-key configuration error instead of silently producing mock outputs. Mock keys are also rejected in production regardless of casing.
...
Successful Stability responses must include non-empty image bytes. Empty image bodies are treated as fatal malformed-provider responses instead of being stored as asset media.
