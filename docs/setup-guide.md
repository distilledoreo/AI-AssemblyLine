...
Run `npm run preflight:production` before release. It verifies required production environment variables, secret/key lengths, real non-mock OpenAI, Stability, and Runway credentials, optional Google/GitHub OAuth client/secret pair consistency, FFmpeg/ffprobe availability, and TCP reachability for Postgres and Redis.
