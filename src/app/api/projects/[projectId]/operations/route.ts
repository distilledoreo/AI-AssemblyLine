...
async function operationsPayload(projectId: string) {
  const [storage, metrics, bundles] = await Promise.all([
    getProjectStorageUsage(projectId),
    getProjectJobMetrics(projectId),
    listExportBundles(projectId),
  ]);
  return {
  ...
  try {
    ...
    if (body.action === "export") {
      ...
      return Response.json(
        { ...(await operationsPayload(projectId)), export: exported },
        { status: 201 },
      );
    }
    if (body.action === "import") {
      ...
      return Response.json(
        { ...(await operationsPayload(projectId)), import: imported },
        { status: 201 },
      );
    }
    assertProjectPermission(await getProjectRole(user.id, projectId), "edit_project_settings");
    if (body.action === "cleanup_orphans") {
      const cleanup = await cleanupOrphanFiles(projectId);
      return Response.json({ ...(await operationsPayload(projectId)), cleanup });
    }
    const thumbnails = await clearThumbnailCache(projectId);
    return Response.json({ ...(await operationsPayload(projectId)), thumbnails });
  } catch (error) {
    return toErrorResponse(error);
  }
}
